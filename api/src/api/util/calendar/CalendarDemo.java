...
public class CalendarDemo {
	...
	public static void main(String[] args) {
		...
		Calendar.getInstance()은 내부적으로 스태틱이 싱글톤 패턴으로 객체를 생성하는 방식이다.
		 * 여러개의 인스턴스를 생성하지 않도록 방지하려는 목적에서 코딩되었다.	
		Calendar date = Calendar.getInstance();
		...
		System.out.print("시간 : ");
		System.out.print(date.get(Calendar.HOUR)+"시간");
		System.out.print(date.get(Calendar.MINUTE)+"분");
		System.out.print(date.get(Calendar.SECOND)+"초");
		...
		System.out.println("현재의 달"+(toString(date));
		// 출력된 결과값 : java.util.GregorianCalendar 이 출력되므로 오버라이딩이 필요하다.
		...
		System.out.println("특정 날짜로 세팅"); //스케줄표쓸때 이걸로 하장ㅋ
		date.set(Calendar.HOUR, 22);
		date.set(Calendar.MINUTE, 30);
	...
	public static String toString(Calendar date) {
		return date.get(Calendar.YEAR)+"년"
				+date.get(Calendar.MONTH)+"월"  // 월은 8부터 시작하므로 +1을 해야한다.
				+date.get(Calendar.DATE)+"일";
	}

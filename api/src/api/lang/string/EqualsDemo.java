...
/*
 * java.lang.String
 * ==은 객체의 레퍼런스 벨류(hash code table) 들 비교한것.
 * 문자열에 대한 비교는 반드시 equals() 메소드를 이용..
 * new를 이용하면 heap 영역에 새로운 메모리를 할당하고 
 * hashcode를 생성한다.
 * */
...
public class EqualsDemo {
	public static void main(String[] args) {
		 ...
		 * - String 클래스 생성자를 이용하여 String 인스턴스를 생성한 경우에는
		 * - new 연산자에 의해 메모리 할당이 이뤄지기 때문에
		 * - 항상 새로운 String 인스턴스가 생성된다.
		 * - 그래서, String타입은 생성자로 객체를 생성하지 않고, 리터럴로 생성한다.
		 * 
		 * 모든 클래스파일(*.class)에는 constant pool 이라는 상수풀(static)이 있어서,
		 * 클래스 내에 사용되는 모든 리터럴과 상수들이 저장되어있다.
		 * 등록하고자 하는 문자열이 상수풀에 이미 존재하는 경우에는 그 문자열의 주소값을 반환한다.
		 */
